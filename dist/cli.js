...
export async function main(argv) {
    ...
    if (command === "show" || command === "events" || command === "runs") {
        ...
        try {
            ...
            if (command === "runs") {
                for (const r of store.runs(id))
                    console.log(`attempt ${r.attempt} · ${r.outcome ?? "running"} · ${r.model ?? "?"} · ${fmtUsd(r.costUsd)} · ${r.finishedAt ? fmtDuration(r.finishedAt - r.startedAt) : "…"}${r.error ? ` · ${r.error.slice(0, 120)}` : ""}`);
                return 0;
            }
            ...
            if (q.dependsOn.length)
                console.log(`depends on: ${q.dependsOn.join(", ")}`);
            if (q.result)
                console.log(`\nresult:\n${q.result}`);
            if (q.error)
                console.log(`\nerror:\n${q.error}`);
    ...
    if (command === "dash" || command === "dashboard") {
        ...
        await new Promise((resolveQuit) => {
            ...
            const component = dashboard({
                ...
                actions: {
                    run: (qid) => {
                        ...
                        if (!claimed)
                            return `cannot claim ${qid} (blocked, scheduled later, in backoff, or running)`;
                        inFlight.add(qid);
                        void dispatchClaimed(runtime, cfg, claimed, () => { })
                            .catch(() => { })
            ...
            const handle = runTui(component, {
                onQuit: () => {
                    if (inFlight.size)
                        console.log(`note: ${inFlight.size} in-flight quest(s) interrupted; they are reclaimable (questline run / daemon).`);
                    runtime.shutdown(cwd);
                    store.close();
                    resolveQuit();
                },
