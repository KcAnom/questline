...
import { executorFor, inspectConfig } from "./config.js";
import { executeQuest } from "./executor.js";
import { ANSI_THEME, dashboard, questDetailLines, questTableText } from "./dashboard.js";
import { runTui } from "./tui.js";
import { QuestRuntime } from "./runtime.js";
import { QuestScheduler } from "./scheduler.js";
...
function parseIntFlag(value, name, min) {
    if (value === undefined)
        return undefined;
    const n = Number(value);
    if (!Number.isInteger(n) || !Number.isFinite(n) || n < min)
...
async function dispatchClaimed(runtime, cfg, claimed, log) {
    ...
    if (result.telemetry)
        runtime.updateTelemetry(lease, result.telemetry);
    runtime.updateTelemetry(lease, { lastActivity: result.status });
    if (result.status === "aborted" || signal.aborted) {
        const reason = signal.reason && typeof signal.reason === "object" && "kind" in signal.reason ? String(signal.reason.kind) : "aborted";
...
function loadOrReportConfig(cwd) {
    try {
        ...
        if (Array.isArray(e.diagnostics))
            console.error(e.diagnostics.map((d) => `${d.file}${d.line ? `:${d.line}${d.column ? `:${d.column}` : ""}` : ""}${d.path ? ` (${d.path})` : ""}: ${d.message}`).join("\n"));
        else
...
export async function main(argv) {
    ...
    try {
        if (command === "add") {
            ...
            if (!role || !task) {
                console.error('usage: questline add <role> "task..." [flags]');
                return 2;
            }
            const notBefore = values["not-before"] ? Date.parse(values["not-before"]) : undefined;
            if (values["not-before"] && Number.isNaN(notBefore)) {
                console.error(`cannot parse --not-before "${values["not-before"]}"`);
                return 2;
            }
        ...
        if (command === "list") {
            ...
            try {
                ...
                if (values.json)
                    ...
                    console.log(rows.length ? questTableText(rows) : "No quests for this project. Add one: questline add <role> \"task\"");
                return 0;
            }
            finally {
                store.close();
        ...
        if (command === "show" || command === "events" || command === "runs") {
            ...
            if (!id) {
                console.error(`usage: questline ${command} <id>`);
                return 2;
            }
            const store = openStore(cfg);
            try {
                const q = store.get(id);
                if (!q || q.project !== resolve(cwd)) {
                    console.error(`no quest ${id} in this project`);
                    return 1;
                }
                if (command === "events") {
                    ...
                    if (values.json)
                        ...
                        for (const e of events)
                            console.log(`${new Date(e.at).toISOString()} · ${e.event} · ${JSON.stringify(e.data)}`);
                    return 0;
                }
                if (command === "runs") {
                ...
                if (values.json)
                    ...
                    console.log("");
                    for (const l of questDetailLines(q, payload.runs, payload.events, payload.artifacts))
                        console.log(l);
                    if (q.result)
                        console.log(`\nresult preview:\n${q.result}`);
                    if (q.error)
        ...
        if (command === "health") {
            ...
            try {
                ...
                if (values.json)
                    ...
                    if (h.project)
                        console.log(`project: ${h.project}${h.paused ? ` · paused (${h.pausedReason ?? "no reason"})` : ""}`);
        ...
        if (command === "daemon") {
            ...
            scheduler.start();
            const code = await new Promise((resolveWait) => {
                const stop = (sig) => {
                ...
                const component = dashboard({
                    ...
                    actions: {
                        run: (qid) => {
                            ...
                            if (!claimed)
                                return `cannot claim ${qid} (blocked, paused, scheduled later, in backoff, or running)`;
                            const p = dispatchClaimed(runtime, cfg, claimed, () => { })
                                .catch(() => { })
        ...
        if (command === "group")
            return handleGroup(rest, cfg, cwd);
        if (command === "pipeline")
            return handlePipeline(rest, cfg, cwd);
        if (command === "recurring")
            return handleRecurring(rest, cfg, cwd);
        if (command === "export" || command === "import" || command === "backup")
            return handleArchive(command, rest, cfg, cwd);
        if (command === "api") {
...
async function handleGroup(rest, cfg, cwd) {
    const [sub, ...args] = rest;
    const store = openStore(cfg);
    try {
        if (sub === "create") {
            ...
            if (!name) {
                console.error("usage: questline group create <name>");
                return 2;
            }
        ...
        if (sub === "list") {
            for (const g of store.listGroups(cwd))
                console.log(`${g.id} · ${g.kind} · ${g.state} · ${g.name} · ${g.questIds.length} quests`);
            return 0;
        }
        if (sub === "show") {
            const g = store.getGroup(args[0], cwd);
            if (!g)
                return 1;
        ...
        if (sub === "cancel") {
            console.log(JSON.stringify(store.cancelGroup(args[0], cwd)));
            return 0;
        }
        if (sub === "retry") {
            console.log(JSON.stringify(store.requeueGroup(args[0], cwd)));
            return 0;
        }
        console.error("usage: questline group create|list|show|cancel|retry");
        return 2;
    }
    finally {
        store.close();
...
async function handlePipeline(rest, cfg, cwd) {
    const [sub, name] = rest;
    if (sub === "list") {
        console.log(Object.keys(cfg.pipelines ?? {}).join("\n") || "No configured pipelines.");
        return 0;
    }
    if (sub === "run") {
        if (!name || !cfg.pipelines?.[name]) {
    ...
    console.error("usage: questline pipeline list|run <name>");
    return 2;
}
async function handleRecurring(rest, cfg, cwd) {
    const [sub, ...args] = rest;
    const store = openStore(cfg);
    try {
        if (sub === "add") {
            ...
            if (!name || !values.cron || !values.role || !values.task) {
                console.error("usage: questline recurring add <name> --cron <expr> --role <role> --task <task>");
                return 2;
            }
            const r = store.createRecurring({ project: cwd, name, cron: values.cron, role: values.role, task: values.task, timezone: values.timezone, catchUp: values["catch-up"], enabled: !values.disabled });
            console.log(`recurring ${r.id} · next ${new Date(r.nextRunAt).toISOString()}`);
            return 0;
        }
        if (sub === "list") {
            for (const r of store.listRecurring(cwd))
                console.log(`${r.id} · ${r.enabled ? "on" : "off"} · ${r.cron} · next ${new Date(r.nextRunAt).toISOString()} · ${r.name}`);
            return 0;
        }
        if (sub === "pause" || sub === "resume") {
            const r = store.setRecurringEnabled(args[0], cwd, sub === "resume");
            console.log(r ? `${r.id} ${r.enabled ? "enabled" : "disabled"}` : "not found");
            return r ? 0 : 1;
        }
        if (sub === "remove") {
            const ok = store.deleteRecurring(args[0], cwd);
            console.log(ok ? "removed" : "not found");
            return ok ? 0 : 1;
        }
        console.error("usage: questline recurring add|list|pause|resume|remove");
        return 2;
    }
    finally {
        store.close();
...
async function handleArchive(command, rest, cfg, cwd) {
    const store = openStore(cfg);
    try {
        if (command === "export") {
            ...
            if (positionals[0]) {
                writeFileSync(positionals[0], text);
                console.log(`exported ${archive.quests.length} quests to ${positionals[0]}`);
            }
            else
                console.log(text);
            return 0;
        ...
        if (command === "import") {
            const file = rest[0];
            if (!file) {
                console.error("usage: questline import <file>");
                return 2;
            }
            const archive = JSON.parse(readFileSync(file, "utf8"));
            const result = store.importArchive(archive, { projectMap: Object.fromEntries((archive.projects ?? []).map((p) => [p, cwd])), conflict: "remap" });
        ...
        if (command === "backup") {
            ...
            if (!dest) {
                console.error("usage: questline backup <sqlite-file> [--force]");
                return 2;
            }
