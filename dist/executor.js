...
import { StreamCollector } from "./artifacts.js";
...
function validateTelemetry(raw) {
    ...
    if (t.model !== undefined)
        out.model = String(t.model).slice(0, 256);
    if (t.provider !== undefined)
        out.provider = String(t.provider).slice(0, 128);
    if (t.tier !== undefined)
        out.tier = String(t.tier).slice(0, 128);
    if (t.lastActivity !== undefined)
        out.lastActivity = String(t.lastActivity).slice(0, 1000);
    if (t.turns !== undefined) {
        if (!Number.isInteger(t.turns) || t.turns < 0)
            throw new Error("telemetry.turns must be a nonnegative integer");
        out.turns = t.turns;
    }
    if (t.tokens !== undefined) {
        if (!Number.isInteger(t.tokens) || t.tokens < 0)
            throw new Error("telemetry.tokens must be a nonnegative integer");
        out.tokens = t.tokens;
    }
    if (t.costUsd !== undefined) {
        if (typeof t.costUsd !== "number" || !Number.isFinite(t.costUsd) || t.costUsd < 0)
            throw new Error("telemetry.costUsd must be nonnegative");
...
function parseProtocolLine(line, collector, state, onTelemetry) {
    if (!line.trim())
        return;
    if (line.length > 1024 * 1024)
        throw new Error("protocol line exceeds 1MiB");
    if (state.resultSeen)
        throw new Error("protocol event after result");
    const envelope = JSON.parse(line);
    if (envelope.protocol !== "questline/1")
        throw new Error("unsupported protocol envelope");
    if (envelope.type === "output") {
        if (typeof envelope.text !== "string")
            throw new Error("output.text must be a string");
        ...
        if (typeof envelope.ok !== "boolean")
            throw new Error("result.ok must be boolean");
        state.resultSeen = true;
        state.resultOk = envelope.ok;
        if (envelope.error !== undefined)
            state.resultError = String(envelope.error);
    }
    else {
        throw new Error(`unknown protocol event type ${String(envelope.type)}`);
    }
}
export function executeQuest(quest, executor, signalOrOptions) {
    ...
    return new Promise((resolvePromise) => {
        ...
        let exitCode = null;
        let escalation;
        let protocolState = { buffer: "", resultSeen: false, telemetry: {} };
        ...
        const finish = async () => {
            ...
            try {
                if (protocolState.protocolError)
                    throw new Error(protocolState.protocolError);
                ...
                if (protocol === "questline-jsonl" && !protocolState.resultSeen)
                    throw new Error("protocol result event missing");
                const outResult = await stdout.finish();
                const errResult = await stderr.finish();
                const artifacts = [outResult.artifact, errResult.artifact].filter(Boolean);
        ...
        child.stdout.on("data", (d) => {
            if (protocol === "questline-jsonl") {
                try {
                    ...
                    if (protocolState.buffer.length > 1024 * 1024)
                        throw new Error("protocol line exceeds 1MiB");
                }
                catch (err) {
        ...
        child.on("close", (code) => {
            exitCode = code;
            if (protocolState.protocolError)
                stderr.write(protocolState.protocolError);
            void finish();
        });
    });
}
