 ...
 */
import { fmtDuration, fmtTokens, fmtUsd, shortModelId } from "./fmt.js";
...
export const QUEST_FILTERS = ["all", "active", "failed", "done", "cancelled"];
const ACTIVE_STATES = new Set(["queued", "running", "interrupted"]);
export function filterQuests(rows, filter, search) {
    const bySearch = search.trim().toLowerCase();
    return rows.filter((q) => {
        if (filter === "active" && !ACTIVE_STATES.has(q.state))
            return false;
        if (filter === "failed" && q.state !== "failed")
            return false;
        if (filter === "done" && q.state !== "done")
            return false;
        if (filter === "cancelled" && q.state !== "cancelled")
            return false;
        if (!bySearch)
            return true;
...
export function questRowText(q, now = Date.now()) {
    const mark = STATE_MARK[q.state] ?? "·";
    const age = fmtDuration(now - q.createdAt);
    const extras = (q.priority ? ` p${q.priority}` : "") +
...
export function questTableText(rows, filter = "all", search = "") {
...
export function questDetailLines(q, runs, events) {
    ...
    if (q.context)
        lines.push(`context: ${q.context.slice(0, 300)}`);
    ...
    if (events.length) {
        ...
        for (const e of events.slice(0, 6))
            lines.push(`  ${new Date(e.at).toISOString()} ${e.event}`);
    }
    return lines;
}
export function dashboard(opts) {
    ...
    const act = (kind, allowed, verb) => {
        ...
        if (!allowed(q)) {
            state.status = `${q.id} is ${q.state} — cannot ${verb}`;
            return;
        }
        const fn = opts.actions?.[kind];
        if (!fn) {
            state.status = `${verb} is not available here`;
            return;
        }
        const err = fn(q.id);
    ...
    return {
        ...
        render(width) {
            ...
            lines.push(QUEST_FILTERS.map((f) => (f === state.filter ? theme.accent(`[${f}]`) : theme.dim(` ${f} `))).join(" ") +
                "  " +
                (state.searchMode
            ...
            if (!rows.length)
                lines.push(theme.muted("  No quests match. (add one: questline add <role> \"task\")"));
            const vp = viewport();
            for (let i = state.offset; i < Math.min(rows.length, state.offset + vp); i++) {
                ...
                const colored = q.state === "failed" ? theme.error(rowText)
            ...
            if (rows.length > vp)
                lines.push(theme.dim(`   … ${state.offset + 1}–${Math.min(rows.length, state.offset + vp)} of ${rows.length}`));
            ...
            if (state.status)
                lines.push(theme.warning(` ${state.status}`));
        ...
        handleInput(data) {
            ...
            switch (data) {
                ...
                case "\n":
                    ...
                    break;
                case "r":
                    act("run", (q) => q.state === "queued" || q.state === "interrupted", "run");
                    break;
                case "R":
                    act("retry", (q) => q.state === "failed" || q.state === "cancelled" || q.state === "interrupted", "retry");
                    break;
                case "x":
                    act("cancel", (q) => q.state === "queued" || q.state === "interrupted", "cancel");
                    break;
                case "q":
                case "\x1b":
                case "\x03":
