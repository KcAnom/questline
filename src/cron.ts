...
function parseField(field: string, min: number, max: number): Set<number> {
  ...
  for (const part of field.split(",")) {
    ...
    if (!Number.isInteger(step) || step < 1) throw new Error(`invalid cron step ${stepPart}`);
    let start: number;
    let end: number;
    if (rangePart === "*") {
      start = min; end = max;
    ...
    if (start < min || end > max || start > end) throw new Error(`cron value ${rangePart} outside ${min}-${max}`);
    for (let n = start; n <= end; n += step) out.add(max === 7 && n === 7 ? 0 : n);
  }
  return out;
}

function parseCron(expr: string): Set<number>[] {
...
export function nextCronTime(expr: string, afterMs: number, timezone = "UTC"): number {
  ...
  while (t <= limit) {
    ...
    if (minutes.has(minute) && hours.has(hour) && months.has(month) && doms.has(dom) && dows.has(dow)) return t;
    t += 60_000;
  }
