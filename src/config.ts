...
export const DEFAULTS: QuestlineConfig = {
...
function formatDiagnostic(d: ConfigDiagnostic): string {
...
function lineColumn(text: string, index: number): { line: number; column: number } {
  ...
  for (let i = 0; i < index; i++) {
    if (text.charCodeAt(i) === 10) { line++; column = 1; }
    else column++;
  }
  return { line, column };
}
...
function jsoncToJson(input: string): string {
  ...
  while (i < input.length) {
    ...
    if (ch === '"') { inString = true; stringQuote = ch; out += ch; i++; continue; }
    if (ch === "/" && next === "/") {
      out += "  "; i += 2;
  ...
  for (let j = 0; j < out.length; j++) {
    ...
    if (ch === ",") {
      ...
      if (out[k] === "}" || out[k] === "]") { cleaned += " "; continue; }
    }
    cleaned += ch;
  }
  return cleaned;
...
function parseJsoncFile(path: string): Record<string, unknown> | undefined {
  ...
  try {
    ...
    const match = /position (\d+)/.exec(String((err as Error).message));
    const loc = match ? lineColumn(raw, Number(match[1])) : {};
    throw new ConfigError([{ file: path, message: `invalid JSONC: ${(err as Error).message}`, ...loc }]);
  }
}
...
function assertKeys(obj: Record<string, unknown>, allowed: string[], file: string, path: string, diagnostics: ConfigDiagnostic[]): void {
  const allowedSet = new Set(allowed);
  for (const k of Object.keys(obj)) {
    if (!allowedSet.has(k)) diagnostics.push({ file, path: path ? `${path}.${k}` : k, message: `unknown config key "${k}"` });
  }
}

function validateInteger(value: unknown, file: string, path: string, diagnostics: ConfigDiagnostic[], min: number, opts: { optional?: boolean; finitePositive?: boolean } = {}): void {
  if (value === undefined) { if (!opts.optional) diagnostics.push({ file, path, message: "is required" }); return; }
...
function validateExecutor(value: unknown, file: string, path: string, diagnostics: ConfigDiagnostic[], requireCommand = false): void {
  if (!isPlainObject(value)) { diagnostics.push({ file, path, message: "must be an object" }); return; }
...
function validatePipelineDefinition(value: unknown, file: string, path: string, diagnostics: ConfigDiagnostic[]): void {
  if (!isPlainObject(value)) { diagnostics.push({ file, path, message: "must be an object" }); return; }
  assertKeys(value, ["description", "steps"], file, path, diagnostics);
  if (value.description !== undefined && typeof value.description !== "string") diagnostics.push({ file, path: `${path}.description`, message: "must be a string" });
  ...
  for (const [stepName, step] of Object.entries(value.steps)) {
    ...
    if (!isPlainObject(step)) { diagnostics.push({ file, path: stepPath, message: "must be an object" }); continue; }
    ...
    if (typeof step.role !== "string" || !step.role) diagnostics.push({ file, path: `${stepPath}.role`, message: "must be a non-empty string" });
    if (typeof step.task !== "string" || !step.task) diagnostics.push({ file, path: `${stepPath}.task`, message: "must be a non-empty string" });
    if (step.name !== undefined && typeof step.name !== "string") diagnostics.push({ file, path: `${stepPath}.name`, message: "must be a string" });
    if (step.context !== undefined && typeof step.context !== "string") diagnostics.push({ file, path: `${stepPath}.context`, message: "must be a string" });
    ...
    if (step.retainUntilConsumed !== undefined && typeof step.retainUntilConsumed !== "boolean") diagnostics.push({ file, path: `${stepPath}.retainUntilConsumed`, message: "must be a boolean" });
    if (step.dependsOn !== undefined) {
      if (!Array.isArray(step.dependsOn) || !step.dependsOn.every((d) => typeof d === "string" && d)) {
        ...
        for (const dep of step.dependsOn) {
          if (dep === stepName) diagnostics.push({ file, path: `${stepPath}.dependsOn`, message: `step "${stepName}" cannot depend on itself` });
          if (!names.has(dep)) diagnostics.push({ file, path: `${stepPath}.dependsOn`, message: `unknown dependency step "${dep}"` });
        }
        graph.set(stepName, step.dependsOn);
      }
    } else graph.set(stepName, []);
  }
  ...
  for (const n of names) if (dfs(n)) break;
}

function validateSourceObject(obj: Record<string, unknown>, file: string, diagnostics: ConfigDiagnostic[]): void {
  ...
  if (obj.executor !== undefined) validateExecutor(obj.executor, file, "executor", diagnostics);
  if (obj.executors !== undefined) {
    if (!isPlainObject(obj.executors)) diagnostics.push({ file, path: "executors", message: "must be an object" });
    else for (const [role, ex] of Object.entries(obj.executors)) validateExecutor(ex, file, `executors.${role}`, diagnostics, true);
  }
  if (obj.scheduler !== undefined) {
    if (!isPlainObject(obj.scheduler)) diagnostics.push({ file, path: "scheduler", message: "must be an object" });
    else {
      ...
      if (obj.scheduler.roleConcurrency !== undefined) {
        if (!isPlainObject(obj.scheduler.roleConcurrency)) diagnostics.push({ file, path: "scheduler.roleConcurrency", message: "must be an object" });
        else for (const [role, cap] of Object.entries(obj.scheduler.roleConcurrency)) validateInteger(cap, file, `scheduler.roleConcurrency.${role}`, diagnostics, 1);
      }
    }
  }
  if (obj.pipelines !== undefined) {
    if (!isPlainObject(obj.pipelines)) diagnostics.push({ file, path: "pipelines", message: "must be an object" });
    else for (const [name, p] of Object.entries(obj.pipelines)) validatePipelineDefinition(p, file, `pipelines.${name}`, diagnostics);
  }
  if (obj.notifications !== undefined) {
    if (!isPlainObject(obj.notifications)) diagnostics.push({ file, path: "notifications", message: "must be an object" });
    else {
...
function validateFinalConfig(cfg: QuestlineConfig): ConfigDiagnostic[] {
  const file = "merged config";
  const diagnostics: ConfigDiagnostic[] = [];
  if (typeof cfg.dbPath !== "string" || !cfg.dbPath) diagnostics.push({ file, path: "dbPath", message: "must be a non-empty string" });
  if (typeof cfg.artifactDir !== "string" || !cfg.artifactDir) diagnostics.push({ file, path: "artifactDir", message: "must be a non-empty string" });
  ...
  if (cfg.executors) for (const [role, ex] of Object.entries(cfg.executors)) validateExecutor(ex, file, `executors.${role}`, diagnostics, true);
  if (cfg.pipelines) for (const [name, p] of Object.entries(cfg.pipelines)) validatePipelineDefinition(p, file, `pipelines.${name}`, diagnostics);
  return diagnostics;
}

export function inspectConfig(cwd: string): LoadedConfig {
  const files: string[] = [];
  const diagnostics: ConfigDiagnostic[] = [];
  ...
  for (const path of [userPath, projectPath]) {
    try {
      ...
      if (parsed) {
        ...
        if (path === userPath) user = parsed;
        else project = parsed;
      }
    } catch (err) {
      if (err instanceof ConfigError) diagnostics.push(...err.diagnostics);
      else diagnostics.push({ file: path, message: String(err) });
    }
  }
  if (diagnostics.length) throw new ConfigError(diagnostics);
  const merged = merge(merge(DEFAULTS, user as Partial<QuestlineConfig>), project as Partial<QuestlineConfig>);
  const finalDiagnostics = validateFinalConfig(merged);
  if (finalDiagnostics.length) throw new ConfigError(finalDiagnostics);
  return { config: merged, files };
}

export function loadConfig(cwd: string): QuestlineConfig {
  return inspectConfig(cwd).config;
}

export function executorFor(cfg: QuestlineConfig, role: string): ExecutorConfig {
